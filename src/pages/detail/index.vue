          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                ...
                <!-- 属性值部分 -->
                <dd :class="{ active: attrvalue.isChecked == '1' }" changepirce="0"
                  v-for="(attrvalue, index) in item.spuSaleAttrValueList" :key="attrvalue.id"
                  @click="changeChecked(item.spuSaleAttrValueList, attrvalue)">
                  {{ attrvalue.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skunum" @change="handel">
...
export default {
  ...
  methods: {
    ...
    async addCar() {
      try {
        await this.$store.dispatch('AddCar', { skuId: this.$route.params.skuid, skuNum: this.skunum })
